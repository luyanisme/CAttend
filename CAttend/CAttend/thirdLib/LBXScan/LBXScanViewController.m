...
DEF_MODEL(AttendModel, attendModel)

- (void)load{
    self.attendModel = [AttendModel modelWithObserver:self];
}

- (void)unload{
    [self.attendModel removeObserver:self];
}
...
- (void)showNextVCWithScanResult:(LBXScanResult*)strResult
{
    ...
    [self.attendModel attend:[UserModel sharedInstance].userInfo.username scheduleId:strResult.strScanned];
}
...
ON_MESSAGE3(API, Attend, msg)
{
    if(msg.sending)
    {
        [bee.ui.appBoard presentLoadingTips:@"签到中..."];
    }
